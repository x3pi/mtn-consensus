import os, re
# We need to replace `Core::new` missing arguments in all tests
tests_dir = 'src/tests'
files = [f for f in os.listdir(tests_dir) if f.endswith('.rs')]
files.extend(['src/test_dag_parser.rs', 'src/core.rs', 'src/core_thread.rs'])

for file in files:
    path = os.path.join(tests_dir, file) if not file.startswith('src/') else file
    if not os.path.exists(path):
        continue
    with open(path, 'r') as f:
        content = f.read()

    # Core::new(..., round_tracker) -> Core::new(..., None, None)
    content = re.sub(
        r'(Core::new\([^;]+\s*,\s*round_tracker(?:\.clone\(\))?\s*)\)',
        r'\1, None, None)',
        content,
        flags=re.M | re.S
    )

    # CommitObserver::new(..., leader_schedule) -> CommitObserver::new(..., 0)
    content = re.sub(
        r'(CommitObserver::new\([^;]+leader_schedule(?:\.clone\(\))?\s*,\s*)\)',
        r'\1 0)',
        content,
        flags=re.M | re.S
    )

    with open(path, 'w') as f:
        f.write(content)
