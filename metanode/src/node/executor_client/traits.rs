...
pub trait TExecutorClient: Send + Sync {
    ...
    // 5. Block Sending
    async fn send_committed_subdag(&self, subdag: &CommittedSubDag, epoch: u64, global_exec_index: u64, leader_address: Option<Vec<u8>>) -> Result<u64>;
    async fn flush_buffer(&self) -> Result<()>;
    async fn send_committed_subdag_direct(&self, subdag: &CommittedSubDag, epoch: u64, global_exec_index: u64, leader_address: Option<Vec<u8>>) -> Result<()>;

    // 6. Block Sync
    async fn sync_blocks(&self, blocks: Vec<proto::BlockData>) -> Result<(u64, u64)>;
    async fn get_blocks_range(&self, start_block: u64, limit: u64) -> Result<Vec<proto::BlockData>>;
    ...
    async fn wait_for_sync_to_block(&self, target_block: u64, timeout_seconds: u64) -> Result<(bool, u64, String)>;
...
impl TExecutorClient for ExecutorClient {
    fn is_enabled(&self) -> bool { ExecutorClient::is_enabled(self) }
    fn can_commit(&self) -> bool { ExecutorClient::can_commit(self) }
    fn storage_path(&self) -> Option<&std::path::Path> { ExecutorClient::storage_path(self) }

    fn circuit_breaker(&self) -> &RpcCircuitBreaker { ExecutorClient::circuit_breaker(self) }
    async fn check_send_circuit_breaker(&self) -> Result<()> { ExecutorClient::check_send_circuit_breaker(self).await }
    async fn record_send_failure(&self) { ExecutorClient::record_send_failure(self).await }
    async fn record_send_success(&self) { ExecutorClient::record_send_success(self).await }
    ...
    async fn send_committed_subdag(&self, subdag: &CommittedSubDag, epoch: u64, global_exec_index: u64, leader_address: Option<Vec<u8>>) -> Result<u64> { ExecutorClient::send_committed_subdag(self, subdag, epoch, global_exec_index, leader_address).await }
    async fn flush_buffer(&self) -> Result<()> { ExecutorClient::flush_buffer(self).await }
    async fn send_committed_subdag_direct(&self, subdag: &CommittedSubDag, epoch: u64, global_exec_index: u64, leader_address: Option<Vec<u8>>) -> Result<()> { ExecutorClient::send_committed_subdag_direct(self, subdag, epoch, global_exec_index, leader_address).await }

    async fn sync_blocks(&self, blocks: Vec<proto::BlockData>) -> Result<(u64, u64)> { ExecutorClient::sync_blocks(self, blocks).await }
    async fn get_blocks_range(&self, start_block: u64, limit: u64) -> Result<Vec<proto::BlockData>> { ExecutorClient::get_blocks_range(self, start_block, limit).await }

    async fn set_consensus_start_block(&self, block_number: u64) -> Result<(bool, u64, String)> { ExecutorClient::set_consensus_start_block(self, block_number).await }
    async fn set_sync_start_block(&self, last_consensus_block: u64) -> Result<(bool, u64, String)> { ExecutorClient::set_sync_start_block(self, last_consensus_block).await }
    async fn wait_for_sync_to_block(&self, target_block: u64, timeout_seconds: u64) -> Result<(bool, u64, String)> { ExecutorClient::wait_for_sync_to_block(self, target_block, timeout_seconds).await }

    async fn initialize_from_go(&self) { ExecutorClient::initialize_from_go(self).await }
}
