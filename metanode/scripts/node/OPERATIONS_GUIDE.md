...
> [!TIP]
> **Không cần build thủ công!** Script `run_all_validator.sh` / `run_all.sh` và `resume_all.sh` sẽ **tự động build** cả Rust và Go binary trước khi chạy. Chỉ cần build thủ công khi muốn kiểm tra lỗi compile hoặc build cross-platform.
...
> [!IMPORTANT]
> `run_all_validator.sh` / `run_all.sh` tự động build cả Rust + Go. Nhưng `run_node.sh` và `resume_node.sh` **không tự build**. Nếu có thay đổi code, chạy lệnh build trước:
...
# Chỉ validators (4 nodes, khuyến nghị cho test)
./run_all_validator.sh

# Hoặc tất cả 5 nodes (bao gồm SyncOnly node 4)
./run_all.sh
```
...
4. Khởi động Go Masters → chờ socket ready
5. Khởi động Go Subs
6. Khởi động Rust Metanodes
