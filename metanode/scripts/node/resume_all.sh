...
for id in 0 1 2 3 4; do
    ...
    tmux new-session -d -s "${GO_MASTER_SESSION[$id]}" -c "$GO_SIMPLE_ROOT" \
        "export GOTOOLCHAIN=go1.23.5 && export XAPIAN_BASE_PATH='$XAPIAN' && export METANODE_STATE_SOCK='/tmp/metanode-state-'$id'.sock' && export METANODE_JMT_STATE_PATH='$METANODE_ROOT/config/storage/node_'$id'/jmt_state' && go run . -config=${GO_MASTER_CONFIG[$id]} >> \"$LOG_DIR/node_$id/go-master-stdout.log\" 2>&1"
...
for id in 0 1 2 3 4; do
    ...
    tmux new-session -d -s "${GO_SUB_SESSION[$id]}" -c "$GO_SIMPLE_ROOT" \
        "export GOTOOLCHAIN=go1.23.5 && export XAPIAN_BASE_PATH='$XAPIAN' && export METANODE_STATE_SOCK='/tmp/metanode-state-'$id'.sock' && export METANODE_JMT_STATE_PATH='$METANODE_ROOT/config/storage/node_'$id'/jmt_state' && go run . -config=${GO_SUB_CONFIG[$id]} >> \"$LOG_DIR/node_$id/go-sub-stdout.log\" 2>&1"
