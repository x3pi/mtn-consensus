...
import (
	...
	"github.com/meta-node-blockchain/meta-node/pkg/hbbft_rbc"
...
func LoadConfigFromFile(filename string) (*hbbft_rbc.NodeConfig, error) {
	data, err := ioutil.ReadFile(filename)
	if err != nil {
		return nil, fmt.Errorf("could not read config file: %w", err)
	}
	var config hbbft_rbc.NodeConfig
...
func main() {
	...
	process, err := hbbft_rbc.NewProcess(allConfig)
	if err != nil {
